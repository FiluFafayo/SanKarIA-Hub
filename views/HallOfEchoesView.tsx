...
const CampaignPortal: React.FC<{ campaign: Campaign, onSelect: () => void, onUpdate: (campaign: Campaign) => void, isMyTurn: boolean }> = ({ campaign, onSelect, onUpdate, isMyTurn }) => {
    ...
    >
        ...
        <button 
            ...
            className={`absolute bottom-3 right-3 text-xs px-3 py-1.5 rounded-full transition-all duration-200 transform ${campaign.isPublished ? 'bg-red-600 hover:bg-red-500 text-white' : 'bg-green-600 hover:bg-green-500 text-white' } opacity-0 group-hover:opacity-100 group-hover:translate-y-0 translate-y-2`}
        >
...
export const HallOfEchoesView: React.FC<HallOfEchoesViewProps> = ({ onClose, campaigns, onSelectCampaign, myCharacters, onUpdateCampaign }) => {
