...
const CreateCharacterWizard: React.FC<{
    onSave: (char: Omit<Character, 'id' | 'ownerId' | 'image'>) => void; // Hapus ownerId dari Omit
    onCancel: () => void;
    ...
    const handleSave = () => {
        ...
        onSave(newCharData); // Kirim data tanpa ownerId
    };
...
export const ProfileModal: React.FC<ProfileModalProps> = ({ onClose, characters, setCharacters, userId, createClassLoadout }) => {
    ...
    const handleCreateCharacter = async (charData: Omit<Character, 'id' | 'ownerId' | 'image'>) => {
        if (!userId) { // Pastikan user sudah login sebelum buat karakter
            alert("Anda harus login untuk membuat karakter.");
            return;
        }
        // Buat objek lengkap DENGAN ownerId di sini
        const newCharBase = {
            ...charData,
            ownerId: userId, // Set ownerId dari prop userId (session.user.id)
            image: `https://picsum.photos/seed/${generateId('charimg')}/100`,
        };
        try {
            // Panggil saveCharacter TANPA ID, biarkan Supabase generate
            // newCharBase tidak punya 'id' tapi punya 'ownerId'
            const savedChar = await dataService.saveCharacter(newCharBase);
            setCharacters(prev => [...prev, savedChar]);
            setSelectedChar(savedChar);
            setIsCreating(false);
        } catch (error) {
            console.error("Gagal menyimpan karakter baru:", error);
            alert("Gagal menyimpan karakter baru ke database.");
        }
    };
